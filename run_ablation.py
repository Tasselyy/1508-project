...
def _run_retrieval_condition(
    ...
    """Run both retrievers on given chunks/queries and save a benchmark log."""
...
def _reset_query_results(queries: list[dict]) -> None:
    """Remove retrieval results from queries so they can be reused across conditions."""
    keys_to_remove = [
...
def _subsample_corpus(corpus_df: pd.DataFrame, size: int, gold_page_ids: set[int], seed: int) -> pd.DataFrame:
    """Subsample corpus: keep all gold pages, randomly sample distractors to reach target size."""
...
def _get_gold_page_ids(sampled_queries: list[dict]) -> set[int]:
    """Extract gold page IDs from ground truth chunk IDs (format: '{wid}_{para_idx}')."""
    ids = set()
    for q in sampled_queries:
...
def run_corpus_scale_ablation(config: dict, test_data: dict, results_dir: str) -> list[dict]:
    """Ablation: vary corpus size by subsampling from cached corpus (no streaming)."""
...
def run_chunk_granularity_ablation(config: dict, test_data: dict, results_dir: str) -> list[dict]:
    """Ablation: vary chunking strategy on the default corpus."""
...
def run_top_k_ablation(config: dict, test_data: dict, results_dir: str) -> list[dict]:
    """Ablation: evaluate recall at different k values on the default test set."""
    k_values = config["dimensions"]["top_k"]["values"]
...
def main():
    ...
    # === 1. Build test set from cached corpus (no Wikipedia streaming) ===
